...
public class TC10_ClassL_ValidationCheck_SubmitCase extends BaseTest
{
	...
	String Screenshotname="ClassLConfirmationScreen";  							//Screen shot name.
	...
	SoftAssert s_assert= new SoftAssert();
		...
		{		
			...
			MyAccountClassL cl=new MyAccountClassL(driver);
			...
			Explicitwait(ct.ClassLServicelink,10);
			ct.ClickonclassLlink();	
			log4j_report.info("User clicked on ClassL SubService ");
			logger.log(LogStatus.INFO, "Customer selected ClassL Sub_SubService");
//			ut.Verify_SubService_titlepage();
			...
			ut.Postcode_SelectAddress(Sh_ClassL,1,2,3);              // Parameter are Sheetname, excel filename & path, Row, Colunm, Dropdown index value (Actual address)  
			cl.Enter_Firstname_CT_Payer(Sh_ClassL,1,4);
			cl.Enter_Lastname_CT_Payer(Sh_ClassL,1,5);
			ut.telphonenumber();
			cl.Enter_Email(Sh_ClassL, 1, 7);
			...
			ut.selectdropdownvalue(cl.property_unfurnished_vacation_Dpd, SelectbyIndex,"2");  // Select dropdown for field "Was the property unfurnished after vacation?"
			cl.Enter_DateOfRepossesion(Sh_ClassL, 1, 10);
			cl.Enter_Bank_Building_repossessed(Sh_ClassL, 1,11);
			cl.Enter_further_Enquiry_ReferenceNumber(Sh_ClassL, 1, 12);
